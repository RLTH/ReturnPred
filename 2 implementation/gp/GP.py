...
monthly, daily, x_var_selection = loading(x_var_selection, ADV_filter)
if daily_monthly:
...
#    sign = np.sign(corr.shift(-3))
    ...
    temp_data_rank = temp_data_rank.groupby(mth, group_keys=False).apply(lambda x: x.sort_values('y'))
    accs = temp_data_rank.groupby(mth, group_keys=False).apply(accuracy_at_k)
    
    return rets, corrs, corr_ranks, accs


#def accuracy_at_k_from_score(temp_data):
#    temp_data_rank = temp_data.groupby(mth).rank(ascending=True)
#    temp_data_rank = temp_data_rank.groupby(mth, group_keys=False).apply(lambda x: x.sort_values('y'))
#    
#    accs = temp_data_rank.groupby(mth, group_keys=False).apply(accuracy_at_k)
...
def expr_print(individual):
    ...
    rets, corrs, corr_ranks, accs = return_from_score(temp_data)
...
pset = gp.PrimitiveSet("MAIN", len(x_var_selection))

#pset.addPrimitive(int_div, [int, series], series)

#pset.addPrimitive(lag, 2)
#pset.addPrimitive(mean, 2)
#pset.addPrimitive(std, 2)
#pset.addPrimitive(gtz, 1)
#pset.addPrimitive(ltz, 1)
...
pset.addEphemeralConstant("rand15_{}".format(time.time()), lambda: random.randint(1, 5))
...
toolbox.register("expr", gp.genGrow, pset=pset, min_=1, max_=2) #genHalfAndHalf
...
#                ret = rets.mean()
                ret = gmean(rets)
                corr = corrs.fillna(0).mean()
                corr_rank = corr_ranks.fillna(0).mean()
                ratio = rets.mean() / rets.std()
                acc = accs.mean()
                
                score_dict[expr] = acc #%
                
                print('{:.4f}, {:.4f}, {}'.format(acc, rets.mean(), str(expr)))
